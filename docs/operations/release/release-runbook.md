...
**Status — 2026-08-16:** M5.2 source commit `5327b6f` passed protected CI/merge, canonical migration 006 schema/grant readback, immutable ACA/Vercel deployment, exact health/readiness/CORS, hosted authenticated acceptance (`6/6`), private-sentinel log sampling, disposable-user cleanup, and application-only rollback/authenticated `4/4` with final candidate restoration. M5.2 is Verified. Rate limiting remains explicitly deferred/accepted, while collector redaction, broad per-signal delivery, and alert routing remain open. Ten genuine zero-replica trials completed on 2026-08-11 with readiness 10/10, p50 34.586 seconds, and p95/max 39.425 seconds; the 30-second p95 criterion was not met. The canonical shared-hosted app uses `minReplicas=1`. The backup limitation remains accepted. The Azure provider-synthetic test path is **NOT VERIFIED**; one alternate temporary metric-alert path has a confirmed receipt but does not prove broad routing. This runbook does not authorize production readiness or M3 completion.
