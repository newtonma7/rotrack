...
**Status — 2026-08-16:** the M5.2 candidate passed immutable ACA/Vercel readback, public smoke, hosted authenticated acceptance (`6/6`), sampled application-log privacy sentinels, cleanup, and application-only rollback with matching prior-commit authenticated `4/4`; final state is the candidate. The application logging boundary, ACA runtime label, and aggregate Log Analytics ingestion are observed, but collector redaction, broad per-signal delivery, and alert routing remain open. One alternate temporary metric-alert path has a confirmed receipt; the provider-synthetic command failed, so provider-synthetic delivery remains **NOT VERIFIED**. Rate limiting remains explicitly deferred/accepted. Ten genuine zero-replica trials completed on 2026-08-11 with readiness 10/10 and p95/max 39.425 seconds, above the 30-second criterion; the canonical shared-hosted app now uses `minReplicas=1`. The backup limitation remains accepted. Every threshold below is a **suggested initial threshold**, not a measured baseline; scale-to-zero applies only to a future/reserved topology decision, while credit-expiry risk still requires explicit signals.
