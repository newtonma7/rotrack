...
3. That environment provides all of these values (never repository-level PR
   ...
   - `ROTRACK_E2E_BASE_URL`, an HTTPS URL for the isolated staging frontend;
   - `ROTRACK_E2E_EXPECTED_API_URL`, the approved HTTPS staging API base ending in `/api/v1`;
   - `ROTRACK_STAGING_SUPABASE_PROJECT_REF`, `ROTRACK_DEVELOPMENT_SUPABASE_PROJECT_REF`, and `ROTRACK_PRODUCTION_SUPABASE_PROJECT_REF`, three distinct protected project identities;
   - `ROTRACK_PRODUCTION_FRONTEND_URL` and `ROTRACK_PRODUCTION_API_URL`, authoritative protected identities that must differ from the staging targets;
...
staging availability cannot be validated from a local checkout. Treat each as a
