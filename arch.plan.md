...
- Pull-request workflows and local guards cover frontend/backend suites, isolated PostgreSQL migration apply/verify, prospective-tree and history secret scanning, operational contract tests, and a credential-free container build. The checked-in authenticated workflow targets logical `nonproduction`, runs only from trusted-default-branch `repository_dispatch`, and is disabled before runner/environment/secret access unless an administrator-controlled variable explicitly enables it. Under the approved solo-maintainer policy, GitHub environment secrets remain empty and authenticated E2E uses local external storage states until a second trusted reviewer is available
- The checked-in AWS staging render/validation files are historical and unselected; they still enforce obsolete three-reference/AWS assumptions. The active hosted deployment uses one shared Supabase project, one Azure Container App boundary, and one Vercel project whose Production deployment is the canonical public frontend. Vercel Preview deployments are not an isolated environment and must not be treated as a production boundary. The single-environment decision and live readback are recorded in [`docs/operations/single-environment.md`](docs/operations/single-environment.md).
...
The product owner explicitly accepted a single shared hosted environment because the Azure subscription cannot provision a second Container Apps managed environment. This is a deliberate availability/data-isolation tradeoff, not production separation.
...
The browser authenticates against the shared Supabase project and sends bearer tokens to the shared Azure API. Credential-free pull-request CI uses isolated PostgreSQL and never connects to hosted data. Hosted smoke and manual testing must use disposable accounts and data. Every deployment changes the same API and database used by the public frontend; a bad release can affect all users, and test data can coexist with user data. The `rotrack-prod` Supabase project and guarded production Azure lane are intentionally not used under this decision. This topology can be revisited later by creating a second Azure quota boundary and migrating data deliberately.

The product-owner-approved solo-maintainer equivalent remains unchanged: `main` requires pull requests with zero human approvals, strict app-bound automated checks, administrator enforcement, linear history, and no force pushes/deletion; `CODEOWNERS` remains advisory. Repository authentication secrets remain empty and the hosted authenticated workflow remains disabled. Read-back and operating rules are recorded in [`docs/operations/single-environment.md`](docs/operations/single-environment.md).
...
- GitHub-hosted green checks, deliberate required-check failure enforcement, branch protection, and protected-environment restrictions are externally observed. PR #18 passed all eight required contexts and merged through protected rebase as `12dfdffb62ee51f6912b290eecc651444e797b90`; PR #19's required `Frontend` context failed with app ID `15368` and its open metadata reported `mergeStateStatus: BLOCKED`. M3.1 is **Verified**. Authenticated deployed smoke and the remaining M3.2/M3.3 operational gates remain open.
- The shared hosted target is the immutable-digest Spring backend in `rotrack-nonproduction` / `rotrack-api-nonproduction`, the shared Supabase Free project, and the Vercel Production alias `rotrack-ecru.vercel.app`. On 2026-08-10 the API health check returned `200`, the canonical Production CORS preflight returned `200` with the exact origin and credentials, unrelated origins returned `403`, and the redeployed Vercel Production build contained the shared API and Supabase project references. The separate `rotrack-prod` Supabase project and production Azure lane remain unused by decision. Source-controlled templates or synthetic validation alone are never cloud evidence.
- The backend promotes the exact tested image digest. `NEXT_PUBLIC_*` values are embedded at build time, so the canonical Vercel Production deployment must be rebuilt after shared-environment URL changes; a Preview build is never promoted as a substitute for a reviewed Production build.
...
The M3 MVP release gate is currently **not met**. Public launch on the shared environment and M4 implementation remain stopped until the backlog records all prerequisite milestone gates as **Verified**, hosted CI/protection evidence, the authorized immutable backend deployment, reviewed Production frontend build provenance, active rate limiting/logging/telemetry/alerts with owners, authenticated hosted smoke, and an exact backend candidate-to-prior rollback rehearsal. The product owner's single-environment risk acceptance changes the topology only; it does not waive these safety gates.
