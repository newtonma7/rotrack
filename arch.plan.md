...
- The checked-in AWS staging render/validation files are historical and unselected; they still enforce obsolete three-reference/AWS assumptions. The active non-production adapter under `deploy/azure/` and `scripts/azure/` uses one shared non-production Supabase identity, one Vercel project with Preview, logical GitHub gates, and separate Azure boundaries. A separate source-controlled production lane under `deploy/azure/production/` and `scripts/azure/production/` targets only the approved production names and requires an explicit production confirmation plus an explicitly selected production subscription; it allows the approved subscription to be shared with non-production only after distinct resource-boundary checks and does not provision resources. Non-production cloud configuration/readback and public GitHub protection/environment/security readbacks are recorded; production remains unverified and untouched
