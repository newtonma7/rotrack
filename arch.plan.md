...
The product-owner-approved solo-maintainer equivalent does not claim two-person review. Before public visibility, protect `main` with pull requests and required automated CI/CodeQL checks, apply the rules to administrators, block force pushes/deletion, and restrict `nonproduction` deployments to protected `main`. `CODEOWNERS` remains advisory. Keep GitHub environment secrets empty and the hosted authenticated workflow disabled; run the authenticated non-production suite from a trusted local operator context with disposable users and external storage states. A future hosted credentialed workflow requires a second trusted reviewer plus environment approval before its administrator-controlled enable variable or secrets may be configured. Read back the available controls against the [GitHub Environments documentation](https://docs.github.com/en/actions/how-tos/deploy/configure-and-manage-deployments/manage-environments). Production deployment remains stopped until the remaining M3 gates pass. The non-production Azure/Vercel resources described in Section 2 are observed; GitHub protection and every production resource/control remain unverified targets.
