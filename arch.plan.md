...
For the zero-user/pre-user boundary, the product owner authorizes a narrow shared-hosted M4 rollout override: migrations `004_user_preferences.sql` and `005_time_entry_history.sql`, deployment of the reviewed M4 application, and hosted M4 acceptance may proceed despite the full M3 production-readiness gate. The unresolved M3 operational risks remain unverified. This does not authorize M5 hosted rollout or broad production-readiness claims. M4 remains **Implemented—unverified** until its rollout evidence passes.
