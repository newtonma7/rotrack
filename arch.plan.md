...
- Pull-request workflows and local guards cover frontend/backend suites, isolated PostgreSQL migration apply/verify, prospective-tree and history secret scanning, operational contract tests, and a credential-free container build. The checked-in authenticated workflow now targets logical `nonproduction`, uses the approved two-project topology, and runs only from trusted-default-branch `repository_dispatch`; remote branch/environment protection remains a prerequisite before any authenticated secrets are configured
