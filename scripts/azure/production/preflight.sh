...
GROUP_LOCATION=$(az group show \
  ...
  --query location \
  --output tsv) || fail 'production resource group does not exist'
...
if workspace.get('name') != 'rotrack-production-logs' or workspace.get('sku') != 'PerGB2018' or workspace.get('retentionInDays') != 30 or workspace.get('dailyQuotaGb') != 0.1:
...
BUDGET_URL="https://management.azure.com/subscriptions/$AZURE_SUBSCRIPTION_ID/resourceGroups/rotrack-production/providers/Microsoft.Consumption/budgets/rotrack-production-budget?api-version=2023-05-01"
BUDGET_JSON=$(az rest \
  --method get \
  --url "$BUDGET_URL" \
  --subscription "$AZURE_SUBSCRIPTION_ID" \
