...
| Pull-request CI source | **Implemented—unverified** | Five credential-free jobs use isolated disposable PostgreSQL and do not touch hosted Supabase. The separately dispatched authenticated workflow still targets `disposable-staging-auth` and legacy three-Supabase-reference checks; hosted green/blocking checks, repository/environment protection, and conversion to target `nonproduction` remain unobserved/open. |
...
| Production-separated non-production deployment | **Blocked** | The approved target is the shared non-production/dev Supabase project for approved authenticated E2E, Vercel Preview in one Vercel project, target logical GitHub `nonproduction`, and separate Azure managed environment `rotrack-nonproduction-env` inside resource group `rotrack-nonproduction` with app `rotrack-api-nonproduction`. It is separated from production but intentionally shared with development/authenticated E2E. No authorized Azure/Vercel deployment or immutable registry digest is evidenced. |
...
**Non-production deployment blocker:** The authorized platform owner must authorize immutable OCI-compatible image publication/deployment to managed environment `rotrack-nonproduction-env` inside `rotrack-nonproduction`, with app `rotrack-api-nonproduction`, configure the shared non-production Supabase and Vercel Preview integrations, and record redacted evidence. Production remains the separate managed environment `rotrack-production-env` inside `rotrack-production`, app `rotrack-api-production`, `rotrack-prod`, and Vercel Production boundary; no cloud resource is claimed as configured or verified.
...
- M3 source adds five credential-free pull-request jobs using isolated PostgreSQL, a separately environment-scoped authenticated-E2E workflow still targeting `disposable-staging-auth` with legacy three-reference checks, isolated PostgreSQL migration apply/verify, prospective-tree/history secret scanning, a non-root Java 21 container, staging render/validation and IAM/runtime-role checks, and release/rollback/monitoring/incident contracts. Conversion to target `nonproduction` remains residual work.
...
An authorized disposable-inbox operator must complete the fresh signup confirmation/first-sign-in evidence for M2.3, or the product owner must record a separate explicit release-gate waiver; the existing exception authorizes M3 source preparation only. In parallel where ownership permits, obtain hosted green/blocking PR and repository-protection evidence; convert the legacy authenticated workflow/scripts to target `nonproduction`; authorize and deploy the shared non-production Supabase/Vercel Preview/Azure managed-environment target at an immutable OCI-compatible image registry digest; complete the trusted fleet-wide/authentication-adjacent rate-limit boundary; and deploy/observe structured logging redaction, telemetry, alerts/routing, cold-start behavior, budget/credit-expiry notifications, Supabase pause/resume ownership, encrypted logical-export retention, restore rehearsal or explicit product-owner risk acceptance, and named incident ownership. Only after those safeguards are active should non-production authenticated smoke 4/4 and exact rollback rehearsal run. Do not start M4 or promote to production until the M3 MVP release gate is **Verified**.
