...
| Production-separated non-production deployment | **Implemented—unverified** | On 2026-08-09 the authorized `eastus2` non-production resource group, managed environment, ACR/identity, Log Analytics cap/retention, budget alerts, digest-bound Container App, and one Vercel Preview were created. HTTPS health/readiness and exact allow/deny CORS passed against the shared non-production Supabase project. Vercel SSO remains enabled and no bypass secret remains. Authenticated 4/4, alert delivery, ten cold starts, backup/restore, and rollback are still required; production was untouched. |
...
- See [`docs/operations/staging/2026-08-09-nonproduction-evidence.md`](docs/operations/staging/2026-08-09-nonproduction-evidence.md). Production was not mutated.
