...
| Production-separated non-production deployment | **Implemented—unverified** | On 2026-08-09 the authorized `eastus2` non-production resource group, managed environment, ACR/identity, Log Analytics cap/retention, budget alerts, digest-bound Container App, and one Vercel Preview were created. HTTPS health/readiness and exact allow/deny CORS passed against the shared non-production Supabase project. The separate production Azure templates/scripts and target-isolation tests are now source-controlled, with strict production/non-production resource-boundary guards and selected-subscription readback; equal subscription IDs remain allowed by the approved plan. This is not cloud evidence. Vercel SSO remains enabled and no bypass secret remains. Authenticated 4/4, alert delivery, ten cold starts, backup/restore, and rollback are still required. No production deployment/environment, `rotrack-prod`, or production Azure mutation occurred; the shared Vercel project protection returned to its prior SSO/no-bypass state. |
...
- Keep production separate in `rotrack-prod`, Vercel Production, managed environment `rotrack-production-env` inside `rotrack-production`, and app `rotrack-api-production`. The source-only production Azure lane under `deploy/azure/production/` and `scripts/azure/production/` requires explicit production confirmation and selected production subscription identity; it permits a shared subscription only with distinct resource-boundary checks and does not claim or perform provisioning.
