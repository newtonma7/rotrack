...
| Pull-request CI source | **Implemented—unverified** | Five credential-free jobs use isolated disposable PostgreSQL and do not touch hosted Supabase. The authenticated workflow uses trusted-default-branch `repository_dispatch`, exact target/storage-state validation, and an administrator-controlled job gate that defaults disabled before runner/environment/secret access. Under the approved solo-maintainer equivalent, GitHub environment secrets stay empty and authenticated E2E runs from a trusted local operator context. Hosted green/blocking checks and public `main` protection remain unobserved/open. |
...
- Missing required evidence: a hosted green PR URL, a deliberately failing required GitHub check that blocks merge, public `main` protection applied to administrators with pull requests/required checks and no force push/deletion, and `nonproduction` restricted to protected `main`. Human approval is not claimed under the approved solo-maintainer equivalent; `CODEOWNERS` remains advisory and GitHub environment secrets remain empty. Keep this task unverified until the available external controls are observed.
