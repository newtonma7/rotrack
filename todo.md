...
**Current release target:** M5.2 Verified; decide M5.3 source scope while full production readiness remains a separate unmet gate
...
|---|---|---|
| Documentation currency | **Verified** | M5.2 implementation, protected delivery, shared-hosted rollout, rollback, and residual boundaries are reconciled across architecture, backlog, setup, and operations documents. |
...
| Notes, logs, friends, groups | **In progress** | M5.1 and M5.2 are Verified. M5.3 logs and later social work remain unstarted and separately gated. |
...
**Hosted dependency:** M4 and the dated M5.2 shared-hosted rollout are **Verified**. M5.3 hosted work and broad production-readiness claims remain unauthorized.
...
- Authenticated browser acceptance — 2026-08-12: the existing authenticated tracker/ownership Playwright suite passed 4/4 against frontend `http://localhost:3001`, backend `http://localhost:8080`, and a disposable loopback PostgreSQL 17.10 database. A focused real-browser M4 run then passed 20-row pagination/load-more, create/edit/delete, `TIME_ENTRY_OVERLAP`, active-entry exclusion, two-user history isolation, and loaded mobile layout without horizontal overflow. It exposed that the editor displayed the saved timezone but converted submitted wall times with the browser timezone; a red component test using a saved zone different from the browser reproduced the defect, and the minimal fix now passes the saved `timeZone` to both instant conversions. Frontend lint, typecheck, 65/65 Vitest tests, and the placeholder-config production build passed afterward. Approved external disposable auth states were copied to temporary mode-`0600` files and rebound to localhost; no hosted database or cloud resource was mutated.
...
**Milestone gate:** **Verified.** M4 is the database/application prerequisite for M5, and the dated M5.2 shared-hosted rollout has passed. Full M3 production readiness remains unmet.
...
**Dependencies:** Milestone 4 and M5.2 **Verified**. M5.3 source/hosted work remains separately gated.
...
- Note cursor pagination/invalid-cursor cases, rich-text canonicalization and ceilings, replay/deleted-replay response and completion-log privacy, and safe error-body omission are covered at the accepted service/controller seams. The subsequent M5.2 hosted migration/schema verification and rollout passed on 2026-08-16.
...
- Independent Standards/Spec/security reviews found no material source blocker after fixes; the complexity review left only an optional four-line deduplication. PR #36 passed every protected required context and merged through the rebase path as source commit `5327b6f`.
...
- Candidate revalidation — 2026-08-16: fresh `npm ci`, zero-high audit, lint, typecheck, 153/153 Vitest tests, placeholder-config production build, nine-test Playwright listing, Java 21 clean test/package (201 tests, 21 expected opt-in skips), guards, Azure/release contracts, secret scan, and diff checks passed. The security review's client/server `mailto` mismatch and pre-deserialization request-size finding were reproduced test-first and fixed; authenticated Notes POST/PUT bodies now have a fixed 1 MiB wire cap before Jackson while the canonical document cap remains 256 KiB. Disposable PostgreSQL 17.10 apply passed 5 tests with 1 expected mode skip; ordered migrations 001–006 applied, and verify/repository/Note service passed 21 tests with 3 expected mode skips.
- Shared-hosted verification — 2026-08-16: the canonical shared PostgreSQL project already contained the directly applied migration 006 schema; read-only preflight confirmed Notes/replay tables, RLS, constraints, and runtime DML grants. The merged backend image was published as Linux/amd64 and deployed by immutable digest with matching service version, `notes-hmac-secret` secretRef, Notes writes enabled, scale `1..1`, exact CORS, and healthy liveness/readiness. Vercel Production built merged commit `5327b6f`, reached READY, and the canonical alias returned `200`.
- Hosted acceptance passed 6/6 for the tracker critical path plus M5.2 Notes workspace: explicit Work/Rot lifecycle, close/reopen restoration, two-user isolation, autosave/reload, conflict recovery, checklist persistence, desktop/mobile journal behavior, and cleanup. The M5.1 API-only spec intentionally refused the hosted target and is not counted as a failure. Disposable Auth users were deleted and an independent follow-up found none remaining; sampled ACA logs omitted Note-content and credential/header sentinels.
- Application-only rollback selected the prior ACA revision and prior Vercel Production deployment with migration 006 retained. Public health/readiness/frontend smoke passed, and the matching prior-commit Playwright harness passed authenticated 4/4. The current backend revision and frontend deployment were restored and finished healthy. The first rollback attempt used the new selector harness against the old UI and failed only on expected markup differences; it restored safely and was superseded by the passing prior-commit rehearsal.
...
M5.2 is Verified. Decide and authorize M5.3 source work before implementation; M5.3 hosted work and broad production-readiness claims remain unauthorized, and unresolved M3 risks stay recorded above.
