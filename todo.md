...
| Single shared hosted deployment | **Implemented—unverified** | The product owner accepted one shared runtime instead of separate Preview/Production infrastructure. The existing `eastus2` Azure Container App, shared Supabase project, and Vercel Production alias are now the canonical hosted environment. On 2026-08-10, API health and exact credentialed Production CORS passed; the Production frontend was rebuilt with the shared API/Supabase values. Hosted authenticated smoke, alert delivery, ten cold starts, backup/restore, and rollback remain required. `rotrack-prod` and the separate production Azure lane remain unused. |
...
**Shared hosted deployment residual gate:** The single Azure/Vercel/Supabase runtime is configured and its decision/readback is recorded in [`docs/operations/single-environment.md`](docs/operations/single-environment.md); the original Azure evidence remains in [`docs/operations/azure-nonproduction.md`](docs/operations/azure-nonproduction.md). The local-only credential-object finding was purged and the shared runtime password rotation/local-and-ACA redeployment passed. Remaining owner/unblock conditions are deployed authenticated 4/4, alert delivery, cold-start trials, backup/restore or risk acceptance, and rollback rehearsal. The separate `rotrack-prod`/production Azure lane is intentionally unused.
...
- Historical evidence — 2026-08-09: temporary public draft PR #17 (`2c34341e6cdea797651a6ba82a217ebeaf0cd703`) was credential-free and contained only an empty commit. All eight required contexts appeared and succeeded, while the checked-in advanced `CodeQL (java)` and `CodeQL (javascript-typescript)` jobs failed because default setup rejects advanced SARIF. PR #17 was closed unmerged and its temporary branch/worktree were deleted; the advanced workflow was subsequently removed.
- Evidence — 2026-08-09, PR #18: normal documentation PR `7ddb8d12811630c6edd83a28d362b172a533b43c` passed all eight required contexts and merged through the protected rebase path as `12dfdffb62ee51f6912b290eecc651444e797b90`; its remote branch and writer worktree/local branch were removed.
- Evidence — 2026-08-09, PR #19: normal TEMP/protection-verification PR `d057f7d4691d3943bdf4b313c0291189fde59625` used one isolated temporary TypeScript file, `frontend/src/__temporary_required_frontend_failure__.ts`, with deterministic TS2322. The exact required `Frontend` context, GitHub Actions app ID `15368`, failed; the other seven required contexts succeeded. While open, read-only PR metadata reported `mergeStateStatus: BLOCKED` (and `mergeable: MERGEABLE`). PR #19 was closed unmerged; its remote/local branch, worktree, and temporary file were removed.
...
- Remaining required evidence: ACA hardening controls, service-version-to-digest deployment binding, official shared-project CA provenance, authenticated hosted browser evidence, ten-trial cold-start observations and replica decision, budget/credit-expiry alert routing, and a redacted completed checklist. The shared API/frontend health and exact Production CORS readback is recorded in `docs/operations/single-environment.md`; no authenticated success is claimed.
...
- Shared-environment launch uses the exact passing backend digest and a separately recorded reviewed Vercel Production build because `NEXT_PUBLIC_*` values are embedded at build time; it must pass production-safe verification.
