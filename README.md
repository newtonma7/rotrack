...
- The approved deployment target is two Supabase Free projects (shared non-production/dev plus `rotrack-prod`), one Vercel project using Preview and Production, logical GitHub `nonproduction`/`production` environments, and separate Azure managed environments/resource groups/apps. The repository is still private and unprotected. Under the approved solo-maintainer equivalent, public `main` must require automated CI/CodeQL checks and block direct/force pushes and deletion, while GitHub environment secrets remain empty and the hosted authenticated workflow stays administrator-disabled. Authenticated non-production E2E uses disposable local external state until a second trusted reviewer exists. See [`docs/operations/azure-nonproduction.md`](docs/operations/azure-nonproduction.md).
