...
- Managed-CA startup, independent liveness/readiness, degraded readiness `503`, and exact CORS behavior are locally verified. On 2026-08-10, the shared Azure Container App returned health `200`, the canonical Vercel Production CORS preflight returned exact credentialed `200`, unrelated origins remained denied, and Vercel Production was rebuilt with the shared API/Supabase values. Hosted authenticated smoke, observed telemetry/alert routing, ten cold-start trials, backup/restore, and rollback rehearsal remain open.
- The approved hosted topology is one shared Supabase Free project, one Azure Container App (`rotrack-api-nonproduction`), and the Vercel Production alias `rotrack-ecru.vercel.app`. Vercel Preview deployments are disposable builds, not a separate environment; `rotrack-prod` and the separate production Azure lane are unused. The repository is public and `main` is protected under the approved solo-maintainer policy: pull requests, zero human approvals, strict app-bound required checks, administrator enforcement, linear history, no force pushes/deletion, and advisory `CODEOWNERS`. Required checks are `Guards and secret scan`, `Frontend`, `Backend`, `Backend container artifact`, `PostgreSQL migrations`, `Analyze (actions)`, `Analyze (java-kotlin)`, and `Analyze (javascript-typescript)`. Repository authentication secrets remain empty and `ROTRACK_AUTHENTICATED_E2E_ENABLED` is absent/default-disabled. Vulnerability reporting, Dependabot security fixes, secret scanning, and push protection are enabled. Hosted E2E uses disposable accounts/data and remains open. See [`docs/operations/single-environment.md`](docs/operations/single-environment.md).
